...
async function getStats() {
  ...
  const [uniqueVisitors, totalVisits, newToday, visitsRecent, eventGroups, templateGroups, ratingAgg, ratingGroups, recentReviews, convGroups, toolRatings, visitors30, lawTotal, lawGroups, lawTypes, lawToday, lawJurs] =
    await Promise.all([
      ...
      }) as Promise<ConvGroup[]>,
      prisma.review.groupBy({ by: ["tool"], _avg: { rating: true }, _count: { _all: true } }) as Promise<ToolRating[]>,
      prisma.visitor.count({ where: { lastSeen: { gte: since30 } } }),
      prisma.lawBuddyUsage.count(),
      prisma.lawBuddyUsage.groupBy({ by: ["status"], _count: { _all: true }, _sum: { resultCount: true }, _avg: { durationMs: true } }) as Promise<LawGroup[]>,
      prisma.lawBuddyUsage.groupBy({ by: ["caseType"], _count: { _all: true } }) as Promise<LawTypeGroup[]>,
      prisma.lawBuddyUsage.count({ where: { createdAt: { gte: startOfToday } } }),
      prisma.lawBuddyUsage.groupBy({ by: ["jurisdiction"], _count: { _all: true } }) as Promise<LawJurGroup[]>,
    ]);
